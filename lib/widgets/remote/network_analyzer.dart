...
class NetworkAnalyzer {
  ...
  static Stream<NetworkAddress> discover2(
    ...
    for (int i = 1; i <= 0xFF; ++i) {
      ...
      f.then((socket) {
        ...
        if (e is! SocketException) return; //throw e;
  ...
  static Stream<NetworkAddress> discover3(
    String subnet,
    int port, {
    Duration timeout = const Duration(seconds: 5),
  }) {
    ...
    for (int i = 1; i <= 0xFF; ++i) {
      final host = '$subnet.$i';
      final future = _ping(host, port, timeout);
      bool success = false;
      future.then((socket) {
        bool exists = socket != null;
        if (exists) socket.destroy();
        out.sink.add(NetworkAddress(host, exists));
        success = true;
      }).catchError((_) {
        success = false;
      });
      if (success) futures.add(future);
    }

    Future.wait<void>(futures).then<void>((_) => out.close()).catchError((_) => out.close());
