...
class DiskSpace {
  ...
  final _wmicRegex = RegExp(r'([A-Z][\S]+)\\[ ]+([0-9]+)[ ]+([0-9]+)', caseSensitive: false, multiLine: true);
  static const _wmicLocation = r'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe';

  // wmic logicalDisk get freespace, size, caption
  static const _wmicArgs = ['-command', 'get-wmiobject', 'Win32_volume', '|', 'select', 'Name,Freespace,Capacity'];
  ...
  Future<void> scan() async {
    ...
    if (io.Platform.isLinux || io.Platform.isMacOS) {
      ...
      final output = (await io.Process.run(_dfLocation, _dfArgs)).stdout as String;

      // Example: /dev/sdb1        107132516   93716396    7931016  93% /
      final matches = (io.Platform.isLinux) ? _dfRegexLinux.allMatches(output).toList() : _dfRegexMacOs.allMatches(output).toList();
    ...
    // Windows code.
      ...
      if (!await io.File(_wmicLocation).exists()) {
        throw NotFoundException('Could not locate wmic binary at $_wmicLocation');
      }

      final output = ((await io.Process.run(_wmicLocation, _wmicArgs)).stdout as String).replaceAll('\r', '');
      final matches = _wmicRegex.allMatches(output).toList();

      final netOutput = ((await io.Process.run(_netLocation, _netArgs)).stdout as String).replaceAll('Microsoft Windows Network', '');
        ...
        matches.map(
          (match) {
            ...
            if (netMatches.any((netMatch) => netMatch.group(1) == devicePath)) {
              mountPath = netMatches.firstWhere((netMatch) => netMatch.group(1) == devicePath).group(2)?.trim() ?? '';
            } else {
    ...
    if (disks.isNotEmpty) {
      disks.sort((disk1, disk2) => disk2.mountPath.length.compareTo(disk1.mountPath.length));
    }
    // List<Disk> sortedDisks = List.from(disks);
    // sortedDisks.sort((disk1, disk2) => disk2.mountPath.length.compareTo(disk1.mountPath.length));
    // _disks = sortedDisks;
  }
  ...
  Disk getDisk(io.FileSystemEntity entity) {
    ...
    throw NotFoundException('Unable to get information about disk which contains ' + entity.path);
  }
}
